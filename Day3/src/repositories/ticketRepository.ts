...
export async function getTickets(query: {
    ...
    if (query.search) {
        ticketQuery = ticketQuery.where((ticket: any) =>
            or(
                ticket.title.ilike(`%${query.search}%`),
                ticket.description.ilike(`%${query.search}%`)
            )
        );
    }
    ...
    let dataQuery = ticketQuery.select("id", "title", "description", "priority", "status", "assignee", "createdAt");
